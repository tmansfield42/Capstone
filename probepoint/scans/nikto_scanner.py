...
# Keywords in port product/service/version strings that identify routers/firewalls.
# Nikto hangs indefinitely on these devices — skip them.
ROUTER_SKIP_KEYWORDS = {
    "openwrt", "uhttpd", "dd-wrt", "tomato",
    "cisco", "juniper", "fortinet",
}
ROUTER_DEVICE_TYPES = {"router", "switch", "firewall", "gateway"}
...
def _is_router_host(host_data: dict) -> bool:
    """
    Returns True if any open port's product/service/version string contains a
    router/firewall keyword, or if iot_classification identifies a router device type.
    Nikto hangs indefinitely on these devices — callers should skip them.
    """
    for pd in host_data.get("ports", {}).values():
        ...
        if any(kw in combined for kw in ROUTER_SKIP_KEYWORDS):
            return True
    iot = host_data.get("iot_classification", {})
    device_type = (iot.get("device_type") or "").lower()
    if any(dt in device_type for dt in ROUTER_DEVICE_TYPES):
        return True
    return False


def run_nikto(ip, port, use_ssl=False, timeout=150):
    """
...
def scan_all_hosts(json_file, timeout=150):
    ...
    for ip, host_data in hosts.items():
        if _is_router_host(host_data):
            print(f"  [~] Skipping Nikto for {ip} — detected as router/firewall/switch")
            continue
        ...
        for port_num, port_data in ports.items():
            ...
            try:
                ...
                print(f"  [-] Nikto timed out for {ip}:{port_num} (>{timeout}s) — skipping")
